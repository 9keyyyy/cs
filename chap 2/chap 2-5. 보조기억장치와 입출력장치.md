...
- DMA는 **직접 메모리에 접근**할 수 있는 입출력 기능
  ...
  - DMA 컨트롤러는 시스템 버스에 연결, 입출력장치들의 장치 컨트롤러들은 **입출력 버스**라는 입출력장치 컨트롤러의 전용 버스와 연결
    ...
    1. CPU가 DMA 컨트롤러에 입출력장치의 주소, 수행할 연산, 연산할 메모리 주소 등의 정보와 함께 **입출력 작업을 명령**
    2. DMA 컨트롤러가 **CPU 대신 장치 컨트롤러와 상호작용**하며 입출력 작업 수행
       - 필요한 경우 메모리에 직접 접근
       - 입출력장치 - 메모리 사이 주고받을 데이터는 CPU 거치지 않음
    3. DMA 컨트롤러는 **입출력 작업 완료 시 CPU에 인터럽트**를 걸어 작업 완료를 알림
   
#### 사이클 스틸링
- 시스템 버스는 동시 사용 불가
  - CPU가 시스템 버스 사용할때 DMA 컨트롤러 시스템 버스 사용불가. 반대로도 마찬가지
- DMA 컨트롤러는 CPU가 시스템 버스를 사용하지 않을 때마다 조금씩 사용 or CPU가 시스템 버스 사용 양보
  - CPU 입장에서는 버스에 접근하는 사이클 도둑 맞은 것 -> 사이클 스틸링이라 부름
 
### 입출력 버스: PCIe (Peripheral Component interconnect express)
PCI라는 입출력 버스의 발전된 형태로, 가장 대중적

#### PCIe와 연결된 장치 성능과 관련해 알아야 할 내용
1. 버전에 따라 최대 속도가 달라질 수 있음
2. 버스가 여러 레인을 이용해 정보를 주고 받을 수 있음
   - 레인(lane)이란 PCIe 버스를 통해 정보를 송수신하는 단위 (통신을 주고받는 선로 수)
